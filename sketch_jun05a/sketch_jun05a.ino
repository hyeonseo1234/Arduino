...
void line_run(void)
{
  ...
  while(flag)
  {
    ...
    if(front_sonar() < 200) // 수정: front_sonar() 함수 호출에 괄호 추가
    {
      //stop
...
{
  int flag = 1; // 수정: flag 변수 초기화
  int line_index;
  // 목표 각도 설정 및 회전
  target_angle = calculate_target_angle(); // 수정: 목표 각도를 계산하는 함수 호출
  turn(target_angle); // 수정: turn 함수 호출
  ...
  {
    line_index = read_line_sensor();
    if(line_index > 0) // 수정: 오타 수정: lne_index -> line_index
    {
      motor_R_control(-50); motor_L_control(-50);
      delay(100);
      motor_R_control(0); motor_L_control(0);
