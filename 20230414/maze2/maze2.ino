...
{
  if( (left_sonar>= 0) && (left_sonar <=150  ) && (right_sonar>= 0) && (right_sonar <=150  ) && (front_sonar>= 0) && (front_sonar <=150  ))
...
void loop()
{
  ...
  { // 좌우 벽만 있을 때 직전
    ...
    motor_B_control(HIGH,100); // 오른쪽 후진
  }
  ...
  { 
    ...
    // 시계방향 90
    Serial.println("Rotate CCW");
    motor_A_control(HIGH,100);  // 오른쪽 직진
    motor_B_control(LOW,100);  // 왼쪽은 후진
    delay(600);               // 일정한 시간 동안 회전
    motor_A_control(HIGH,0);  // 오른쪽 정지
    motor_B_control(LOW ,0);  // 왼쪽은 정지
    delay(1000);
  }
  ...
  { 
   ...
   // 90도 회전 반시계 방향으로 회전
    Serial.println("Rotate CCW");
    motor_A_control(LOW,100);  // 오른쪽 직진
